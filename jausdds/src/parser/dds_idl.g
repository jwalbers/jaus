// Grammar for CORBA 3.2 DDS IDL 3
//
// Used CORBA 3.x IDL for ANTLR3: http://www.antlr.org/grammar/1203688376660/Idl.g as baseline.
//
// $Id: dds_idl.g 109 2013-08-27 00:57:44Z jalbers $
//
// Elements like #pragma and #include should really be handled by a preprocessor.
// Keep them in the grammar for now so we have an easier way to validate the IDL files.
...
module
	:	'module' ID '{' definition+ (pragma)? '}'
	;

// OpenSplice pragma, needs review per OpenSplice IDL grammar spec.
pragma
    :   '#pragma' ( ID | ID '.' ID )*
    ;
...
// OpenSplice allows typedefs inside of struct_type body.
member
	:	( (type_spec declarators)
          | ('typedef' type_decl ) ) ';'
	;
...
// Is adding octet_type illegal w/ respect to DDS IDL?
// OpenSplice seems to accept it.
switch_type_spec
	:	integer_type
	|	char_type
    |   octet_type
...
// Added initialized enumerator per OpenSplice DDS IDL.	
enumerator
	:	ID  |  ID '=' INTEGER_LITERAL
	;
